...
function QuickAction({
  ...
  return (
    ...
    >
      <div style={{ color: color || (isDark ? '#00e5cc' : 'var(--accent-blue, #1d4ed8)') }}>
        {icon}
      </div>
      <span
        ...
        <div className="flex items-center gap-2">
          {parsed && (
            <div
              ...
              style={{ border: isDark ? '1px solid #3f3f46' : '1px solid #e5e5e5' }}
            >
              <button
                        ...
                        background:
                          idx % 2 === 0
                            ...
                            : isDark
                              ...
                              : '#fafafa',
                        borderBottom: isDark ? '1px solid #27272a' : '1px solid #f0f0f0',
                      }}
                    >
                      {columns.map((col) => (
                        <td
                        ...
                        >
                          {row[col] === null ? (
                            <span style={{ color: isDark ? '#52525b' : '#999999' }}>null</span>
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <span
          ...
          style={{ color: isDark ? 'var(--accent-cyan, #00e5cc)' : 'var(--accent-blue, #1d4ed8)' }}
        >
          shell&gt;
        </span>
        <input
        ...
        {!hasRun ? (
          <div className="flex items-center justify-center h-full">
            <span
              ...
              style={{ color: isDark ? '#52525b' : '#cccccc' }}
            >
              {'>'}_ Type a command above
            </span>
          </div>
        ) : (
      ...
      <div className="space-y-1.5 max-h-[120px] overflow-auto">
        {gestureActions.length === 0 ? (
          ...
          gestureActions.map((action, idx) => (
            ...
            >
              <span
                className="text-[9px] font-bold uppercase w-20"
                style={{ color: isDark ? '#00e5cc' : 'var(--accent-blue, #1d4ed8)' }}
              >
                #{idx + 1} {action.type}
              </span>
              {(action.type === 'move' || action.type === 'pointerDown') && (
                <>
                  ...
                  <input
                    type="number"
                    value={action.x ?? 0}
                    onChange={(e) => updateAction(idx, { x: parseInt(e.target.value) || 0 })}
                  ...
                  <input
                    type="number"
                    value={action.y ?? 0}
                    onChange={(e) => updateAction(idx, { y: parseInt(e.target.value) || 0 })}
                    className="w-16 px-1.5 py-0.5 rounded text-[10px] font-mono"
                    style={{
                      background: isDark ? '#1f1f23' : '#ffffff',
                      color: isDark ? '#e4e4e7' : '#1a1a1a',
              ...
              {(action.type === 'move' || action.type === 'pause') && (
                <>
                  ...
                  <input
                    ...
                    onChange={(e) => updateAction(idx, { duration: parseInt(e.target.value) || 0 })}
              ...
              <button
                onClick={() => setGestureActions(gestureActions.filter((_, i) => i !== idx))}
...
function ExecuteScript({ onClose, platform }: { onClose: () => void; platform: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {result && (
        <div
          className="p-2 rounded-lg text-[10px] font-mono whitespace-pre-wrap"
          style={{
            background: result.success
              ? isDark
                ? 'rgba(16,185,129,0.1)'
...
function AppsSection({ isDark }: { isDark: boolean }) {
  ...
  const { data: packages = [], isLoading: loadingPkgs } = useInstalledPackages(
    packagesLoaded,
    selectedDevice
  );
  ...
  return (
    <div className="flex gap-3 h-full min-h-0">
      {/* Left: Package List */}
      <div
        className="w-48 flex flex-col rounded-xl overflow-hidden"
        style={{
      ...
      >
        ...
        >
          ...
          {!packagesLoaded && !loadingPkgs && (
            <button
              onClick={() => setPackagesLoaded(true)}
              className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase"
              style={{
                background: isDark ? '#00e5cc' : 'var(--accent-blue, #1d4ed8)',
                color: '#000',
              }}
            >
              Load
      ...
      <div className="flex-1 overflow-y-auto">
        {!selectedPackage ? (
          ...
          <div className="space-y-3">
            ...
            >
              <div className="min-w-0">
                ...
                >
                  {platform === 'ios'
                    ? appInfo.displayName || appInfo.packageName
                    : appInfo.packageName}
                </h3>
                <p
            ...
            <div className="grid grid-cols-2 gap-2">
              {(platform === 'ios'
                ? [
                    ...
                    {
                      label: 'Build',
                      value: appInfo.versionCode ? String(appInfo.versionCode) : '—',
                    },
          ...
          <div
            ...
            style={{
              background: isDark ? '#1f1f23' : '#fef2f2',
              border: isDark ? '1.5px solid #fb7185' : '1.5px solid #dc2626',
              color: isDark ? '#fb7185' : '#dc2626',
            }}
          >
            <span className="text-[10px] font-bold">
              {platform === 'ios' ? 'App not found' : 'Package not found'}
            </span>
          </div>
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label
                  className="px-3 py-2 rounded-xl text-[10px] font-bold uppercase cursor-pointer"
                  style={{
                    background: isDark ? '#1f1f23' : '#f0f0f0',
                    color: isDark ? '#a1a1aa' : '#666666',
                    border: `1.5px solid ${isDark ? '#3f3f46' : '#e5e5e5'}`,
                  }}
                >
                  APK
                  <input
        ...
        {activeSection === 'apps' && <AppsSection isDark={isDark} />}

        {/* ADB Shell Section */}
        {activeSection === 'adb' && <AdbSection onResult={setCommandResult} />}
