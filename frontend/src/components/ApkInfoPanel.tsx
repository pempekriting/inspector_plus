...
import { useInstalledPackages, useAppInfo, apiFetch } from '../services/api';
...
export function ApkInfoPanel({ isDark }: { isDark: boolean }) {
  ...
  const {
    ...
    refetch,
  } = useInstalledPackages(packagesLoaded, selectedDevice);
      ...
      >
        ...
        >
          ...
          >
            {loadingPkgs
              ? 'Loading...'
              : `${filteredPackages.length} ${isIOS ? 'apps' : 'packages'}`}
          </span>
            ...
            <div className="grid grid-cols-2 gap-2">
              {isIOS
                ? [
                    ...
                    ...(appInfo.versionCode
                      ? [{ label: 'Build', value: String(appInfo.versionCode) }]
                      : []),
                    ...
                    ...(appInfo.bundleIdentifier
                      ...
                      : []),
                    ...(appInfo.installType
                      ? [{ label: 'Install Type', value: appInfo.installType }]
                      : []),
                    ...(appInfo.architectures
                      ? [{ label: 'Architecture', value: appInfo.architectures }]
                      : []),
                  ].map(({ label, value }) => (
                    <div
                      key={label}
                ...
                : [
                    ...
                    {
                      label: 'Min SDK',
                      value: `${appInfo.minSdk} (${getSdkName(appInfo.minSdk)})`,
                    },
                    {
                      label: 'Target SDK',
                      value: `${appInfo.targetSdk} (${getSdkName(appInfo.targetSdk)})`,
                    },
                  ].map(({ label, value }) => (
                    <div
                      key={label}
          ...
          <div className="p-4">
            ...
            >
              <span className="text-[10px] font-bold">
                {isIOS ? 'App not found' : 'Package not found'}
              </span>
            </div>
          </div>
        )}
