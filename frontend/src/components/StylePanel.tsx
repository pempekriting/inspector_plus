...
function Chip({
  ...
  return (
    ...
    >
      ...
      <span className="text-[10px] font-mono" style={{ color: isDark ? '#a1a1aa' : '#4a4a4a' }}>
        {value}
      </span>
    </div>
  );
...
export const LayoutChips = memo(function LayoutChips({ styles, compact = true }: LayoutChipsProps) {
  ...
  if (!styles) {
    return (
      <div className="flex items-center gap-1">
        ...
        <span className="text-[10px]" style={{ color: isDark ? '#3f3f46' : '#cccccc' }}>
          —
        </span>
      </div>
    );
  ...
  const hasAnyStyle =
    styles.backgroundColor || styles.textColor || styles.padding || styles.elevation;
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap">
      ...
      />
      {paddingValue && <Chip label="pad" value={paddingValue} isDark={isDark} />}
      {styles.elevation && <Chip label="elev" value={styles.elevation} isDark={isDark} />}
    </div>
  );
});
      ...
      <div className="space-y-0.5">
        {styles.backgroundColor && (
          <div
            ...
            style={{ borderLeft: isDark ? '3px solid #3f3f46' : '3px solid #1a1a1a' }}
          >
            <span
