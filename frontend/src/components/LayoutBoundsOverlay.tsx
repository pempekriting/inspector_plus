    ...
    >
      {items.map(({ label, color }) => (
        <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 3 }}>
          ...
          />
          <span style={{ color: isDark ? '#71717a' : '#666666', whiteSpace: 'nowrap' }}>
            {label}
          </span>
        </div>
      ))}
  ...
  if (!uiTree) {
    return createPortal(
      <EmptyState
        ...
        subtitle="Connect a device and refresh to load layout"
        isDark={isDark}
      />,
      document.body
    );
  ...
  if (!layout) {
    return createPortal(
      <EmptyState
        ...
        subtitle="Capturing screenshot to show layout boxes"
        isDark={isDark}
      />,
      document.body
    );
