 ...
 */
export function generateLocators(node: UiNode, siblingIndex: number = 0): Locator[] {
  ...
  if (node.resourceId) {
    ...
    locators.push(make('android_uiautomator', `UiSelector().resourceId("${node.resourceId}")`, 4));
  }
  ...
  if (node.contentDesc) {
    ...
    locators.push(
      make('android_uiautomator', `UiSelector().description("${node.contentDesc}")`, 4)
    );
  }
  ...
  if (node.text) {
    ...
    locators.push(make('android_uiautomator', `UiSelector().text("${node.text}")`, 3));
    locators.push(make('android_uiautomator', `UiSelector().textContains("${node.text}")`, 2));
  }
...
export function bestLocator(locators: Locator[]): Locator | null {
  ...
  return locators.reduce((best, curr) => (curr.stability > best.stability ? curr : best));
}
