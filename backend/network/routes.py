...
async def stop_vpn_proxy(udid: Optional[str] = Query(None)):
...
async def vpn_status(udid: Optional[str] = Query(None)):
...
@router.get("/traffic")
async def get_traffic(since: float = 0, udid: Optional[str] = None):
...
@router.get("/info")
async def network_info(udid: Optional[str] = None):
...
async def traffic_stream(websocket: WebSocket, udid: Optional[str] = Query(None)):
